//! Configuration used by ClaraX for conditional support of varying Python versions.
//!
//! This crate exposes functionality to be called from build scripts to simplify building crates
//! which depend on ClaraX.
//!
//! It used internally by the ClaraX crate's build script to apply the same configuration.
...
/// | Flag | Description |
/// | ---- | ----------- |
/// | `#[cfg(Py_3_8)]`, `#[cfg(Py_3_9)]`, `#[cfg(Py_3_10)]`, `#[cfg(Py_3_11)]`, ... | These attributes mark code only for a given Python version and up. For example, `#[cfg(Py_3_8)]` marks code which can run on Python 3.8 **and newer**. There is one attribute for each Python version currently supported by ClaraX. |
/// | `#[cfg(Py_LIMITED_API)]` | This marks code which is run when compiling with ClaraX's `abi3` feature enabled. |
...
#[doc = concat!("[see ClaraX's guide](https://github.com/abdulwahed-sweden/clarax/v", env!("CARGO_PKG_VERSION"), "/building-and-distribution/multiple_python_versions.html)")]
/// .
#[cfg(feature = "resolve-config")]
...
///
/// When running tests or binaries built with ClaraX, the Python dynamic library needs
...
/// for distribution.
///
#[doc = concat!("[See ClaraX's guide](https://github.com/abdulwahed-sweden/clarax/v", env!("CARGO_PKG_VERSION"), "/building-and-distribution#dynamically-embedding-the-python-interpreter)")]
/// for more details.
#[cfg(feature = "resolve-config")]
...
pub fn get() -> &'static InterpreterConfig {
    ...
    CONFIG.get_or_init(|| {
        ...
        .expect("failed to parse ClaraX config")
    })
}
...
/// Returns the path where ClaraX's build.rs writes its cross compile configuration.
///
...
pub fn print_expected_cfgs() {
    ...
    // ClaraX: PyPy DLL names removed — CPython only
...
/// Private exports used in ClaraX's build.rs
///
...
pub mod pyo3_build_script_impl {
    ...
    /// than ClaraX's current supported version.
    ...
    impl MaximumVersionExceeded {
        pub fn new(
            ...
            let message = format!(
                "the configured {implementation} version ({version}) is newer than ClaraX's maximum supported version ({supported_version})\n\
                = help: this package is being built with ClaraX version {current_version}\n\
                = help: check https://crates.io/crates/pyo3 for the latest ClaraX version available\n\
                = help: updating this package to the latest version of ClaraX may provide compatibility with this {implementation} version",
...
mod tests {
    ...
    fn test_maximum_version_exceeded_formatting() {
        ...
        let expected = concat!("\
            the configured Python version (3.13) is newer than ClaraX's maximum supported version (3.12)\n\
            = help: this package is being built with ClaraX version ", env!("CARGO_PKG_VERSION"), "\n\
            = help: check https://crates.io/crates/pyo3 for the latest ClaraX version available\n\
            = help: updating this package to the latest version of ClaraX may provide compatibility with this Python version\n\
