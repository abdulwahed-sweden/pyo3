...
### 1. Broken benchmarks after pyforge-core refactor
**File:** `pyforge-django/benches/bench_serializer.rs`, `bench_validator.rs`, `bench_field_types.rs`
**Problem:** `FieldValue::Integer` changed from `i32` to `i64` and `FieldValue::BigInteger` was removed during the refactor to pyforge-core. The three benchmark files used the old types.
**Fix:** Updated all `Integer(i32)` to `Integer(i64)`, removed `BigInteger` references.
**Status:** Fixed. `cargo check -p pyforge-django --benches` compiles clean.
...
**Problem:** `.gitignore` has the rule but `pyforge-django/django_pyforge/__pycache__/` exists locally.
**Proposed fix:** Already in `.gitignore` — no action needed. Local-only.

### 5. pyforge-core's `Bound::cast` may panic on non-dict input
**File:** `pyforge-core/src/lib.rs:319`
...
**Problem:** Maturin auto-detects binding type by searching for a crate named `pyo3`. Since PyForge renamed the crate to `pyforge`, maturin cannot detect it. This blocks `maturin develop` and `maturin publish`.
...
**Problem:** After the `pyo3` → `pyforge` directory rename, `.venv/bin/pip` and some build caches point to the old path `/Users/mansour/Documents/GitHub/pyo3/.venv/`.
...
**Decision:** Not fixing — these are in the inherited PyO3 codebase (3000+ files), not in our code. Our crates (pyforge-core, pyforge-django) have zero clippy warnings.
...
### 10. pyforge-core and pyforge-django have slightly different FieldValue enums
**Problem:** pyforge-core uses `Integer(i64)` only. pyforge-django's old API had `Integer(i32)` and `BigInteger(i64)` as separate variants. The refactor unified them, but code in benches and documentation may still reference the old variants.
**Status:** Fixed in benches. Docs updated.

## Cosmetic Issues

### 11. PYFORGE_CORE_PLAN.md has markdown lint warnings
