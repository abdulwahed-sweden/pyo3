...
//!
//! Uses pyforge-core's `FieldValidationError` for field-level errors.
//! `DjangoError` wraps these with Django-specific variants.

use pyforge::exceptions::{PyTypeError, PyValueError};
use pyforge::PyErr;

// Re-export from pyforge-core — single source of truth for field validation errors.
pub use pyforge_core::error::FieldValidationError;
