...
pub enum ClaraXConstAttribute {
    Name(NameAttribute),
}

impl Parse for ClaraXConstAttribute {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        let lookahead = input.lookahead1();
        if lookahead.peek(attributes::kw::name) {
            input.parse().map(ClaraXConstAttribute::Name)
        } else {
            Err(lookahead.error())
        }
    }
        ...
        take_attributes(attrs, |attr| {
            if attr.path().is_ident("classattr") {
                ...
                for pyo3_attr in pyo3_attributes {
                    match pyo3_attr {
                        ClaraXConstAttribute::Name(name) => attributes.set_name(name)?,
                    }
                }
                Ok(true)
            } else {
