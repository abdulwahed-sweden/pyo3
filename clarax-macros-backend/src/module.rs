...
impl Parse for PyModuleOptions {
    fn parse(input: ParseStream<'_>) -> syn::Result<Self> {
        ...
        options.add_attributes(
            Punctuated::<PyModuleClaraXOption, syn::Token![,]>::parse_terminated(input)?,
        )?;
...
impl PyModuleOptions {
    ...
    fn add_attributes(
        &mut self,
        attrs: impl IntoIterator<Item = PyModuleClaraXOption>,
    ) -> Result<()> {
        ...
        attrs
            ...
            .map(|attr| {
                match attr {
                    PyModuleClaraXOption::Crate(krate) => set_option!(krate),
                    PyModuleClaraXOption::Name(name) => set_option!(name),
                    PyModuleClaraXOption::Module(module) => set_option!(module),
                    PyModuleClaraXOption::Submodule(submodule) => set_option!(
                        submodule,
                        " (it is implicitly always specified for nested modules)"
                    ),
                    PyModuleClaraXOption::GILUsed(gil_used) => {
                        set_option!(gil_used)
                    }
                }
        ...
        match item {
            ...
            Item::Struct(item_struct) => {
                    ...
                    )
                {
                    ...
                    if !has_pyo3_module_declared::<PyClassClaraXOption>(
                        &item_struct.attrs,
                        "pyclass",
                        |option| matches!(option, PyClassClaraXOption::Module(_)),
                    )? {
            ...
            Item::Mod(item_mod) => {
                    ...
                    )
                {
                    ...
                    if !has_pyo3_module_declared::<PyModuleClaraXOption>(
                        &item_mod.attrs,
                        "pymodule",
                        |option| matches!(option, PyModuleClaraXOption::Module(_)),
                    )? {
            ...
            if let Some((pyfn_span, pyfn_args)) = get_pyfn_attr(&mut func.attrs)? {
                ...
                let statements: Vec<syn::Stmt> = syn::parse_quote_spanned! {
                    ...
                    #wrapped_function
                    {
                        ...
                        #[deprecated(note = "`pyfn` will be removed in a future ClaraX version, use declarative `#[pymodule]` with `mod` instead")]
                        #[allow(dead_code)]
...
impl Parse for PyModuleClaraXOption {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        let lookahead = input.lookahead1();
        if lookahead.peek(attributes::kw::name) {
            input.parse().map(PyModuleClaraXOption::Name)
        } else if lookahead.peek(syn::Token![crate]) {
            input.parse().map(PyModuleClaraXOption::Crate)
        } else if lookahead.peek(attributes::kw::module) {
            input.parse().map(PyModuleClaraXOption::Module)
        } else if lookahead.peek(attributes::kw::submodule) {
            input.parse().map(PyModuleClaraXOption::Submodule)
        } else if lookahead.peek(attributes::kw::gil_used) {
            input.parse().map(PyModuleClaraXOption::GILUsed)
        } else {
            Err(lookahead.error())
        }
    }
