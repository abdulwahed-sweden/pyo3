...
use crate::utils::{has_attribute, has_attribute_with_namespace, Ctx, ClaraXCratePath};
use crate::{
...
enum PyImplClaraXOption {
    Crate(CrateAttribute),
}

impl Parse for PyImplClaraXOption {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        let lookahead = input.lookahead1();
        if lookahead.peek(syn::Token![crate]) {
            input.parse().map(PyImplClaraXOption::Crate)
        } else {
            Err(lookahead.error())
        }
    }
...
impl PyImplOptions {
    pub fn from_attrs(attrs: &mut Vec<syn::Attribute>) -> Result<Self> {
        ...
        for option in take_pyo3_options(attrs)? {
            match option {
                PyImplClaraXOption::Crate(path) => options.set_crate(path)?,
            }
        }
...
fn check_pyfunction(pyo3_path: &ClaraXCratePath, meth: &mut ImplItemFn) -> syn::Result<()> {
    let mut error = None;
