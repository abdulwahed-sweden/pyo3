...
/// Check if the given type `ty` is `clarax::Python`.
...
impl Ctx {
    pub(crate) fn new(attr: &Option<CrateAttribute>, signature: Option<&syn::Signature>) -> Self {
        let pyo3_path = match attr {
            Some(attr) => ClaraXCratePath::Given(attr.value.0.clone()),
            None => ClaraXCratePath::Default,
        };
...
#[derive(Clone)]
pub enum ClaraXCratePath {
    Given(syn::Path),
    Default,
}

impl ClaraXCratePath {
    pub fn to_tokens_spanned(&self, span: Span) -> TokenStream {
        match self {
            Self::Given(path) => quote::quote_spanned! { span => #path },
            Self::Default => quote::quote_spanned! {  span => ::clarax },
        }
    }
}

impl quote::ToTokens for ClaraXCratePath {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match self {
            Self::Given(path) => path.to_tokens(tokens),
            Self::Default => quote::quote! { ::clarax }.to_tokens(tokens),
        }
    }
}
...
pub(crate) fn has_attribute_with_namespace(
    attrs: &[syn::Attribute],
    crate_path: Option<&ClaraXCratePath>,
    idents: &[&str],
) -> bool {
    let mut segments = vec![];
    if let Some(c) = crate_path {
        match c {
            ClaraXCratePath::Given(paths) => {
                for p in &paths.segments {
                    segments.push(IdentOrStr::Ident(p.ident.clone()));
                }
            }
            ClaraXCratePath::Default => segments.push(IdentOrStr::Str("clarax")),
        }
    };
    for i in idents {
