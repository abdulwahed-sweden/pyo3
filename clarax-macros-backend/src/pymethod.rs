...
pub(crate) fn impl_py_class_attribute(
    ...
    ensure_spanned!(
        args.is_empty(),
        args[0].ty().span() => "#[classattr] can only have one argument (of type clarax::Python)"
    );
...
fn impl_call_setter(
    ...
    if args.is_empty() {
        ...
        bail_spanned!(
            args[1].ty().span() =>
            "setter function can have at most two arguments ([clarax::Python,] and value)"
        );
    }
...
fn impl_call_getter(
    ...
    ensure_spanned!(
        args.is_empty(),
        args[0].ty().span() => "getter function can only have one argument (of type clarax::Python)"
    );
...
fn impl_call_deleter(
    ...
    if !args.is_empty() {
        bail_spanned!(spec.name.span() =>
            "deleter function can have at most one argument ([clarax::Python,])"
        );
    }
...
/// Split an argument of clarax::Python from the front of the arg list, if present
