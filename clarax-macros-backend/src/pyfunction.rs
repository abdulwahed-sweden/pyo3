...
impl PyFunctionArgClaraXAttributes {
    /// Parses #[pyo3(from_python_with = "func")]
    pub fn from_attrs(attrs: &mut Vec<syn::Attribute>) -> syn::Result<Self> {
        let mut attributes = PyFunctionArgClaraXAttributes {
            from_py_with: None,
            cancel_handle: None,
        };
        take_attributes(attrs, |attr| {
            if let Some(pyo3_attrs) = get_pyo3_options(attr)? {
                for attr in pyo3_attrs {
                    match attr {
                        PyFunctionArgClaraXAttribute::FromPyWith(from_py_with) => {
                            ensure_spanned!(
                        ...
                        PyFunctionArgClaraXAttribute::CancelHandle(cancel_handle) => {
                            ensure_spanned!(
...
pub fn impl_wrap_pyfunction(
    ...
    // ClaraX: async support is always enabled (no feature gate required)
