...
use crate::{
    ...
    pyfunction::{
        FunctionSignature, PyFunctionArgClaraXAttributes, PyFunctionOptions, SignatureAttribute,
    },
    quotes,
    ...
    pub fn parse(arg: &'a mut syn::FnArg) -> Result<Self> {
        match arg {
            ...
            syn::FnArg::Typed(cap) => {
                ...
                let PyFunctionArgClaraXAttributes {
                    from_py_with,
                    cancel_handle,
                } = PyFunctionArgClaraXAttributes::from_attrs(&mut cap.attrs)?;
                ...
                if cancel_handle.is_some() {
                    // `PyFunctionArgClaraXAttributes::from_attrs` validates that
