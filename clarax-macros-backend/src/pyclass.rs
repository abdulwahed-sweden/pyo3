...
pub struct PyClassArgs {
    ...
    pub options: PyClassClaraXOptions,
}
...
impl PyClassArgs {
    fn parse(input: ParseStream<'_>, kind: PyClassKind) -> Result<Self> {
        Ok(PyClassArgs {
            class_kind: kind,
            options: PyClassClaraXOptions::parse(input)?,
        })
    }
...
pub struct PyClassClaraXOptions {
...
impl Parse for PyClassClaraXOption {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        let lookahead = input.lookahead1();
        if lookahead.peek(Token![crate]) {
            input.parse().map(PyClassClaraXOption::Crate)
        } else if lookahead.peek(kw::dict) {
            input.parse().map(PyClassClaraXOption::Dict)
        } else if lookahead.peek(kw::eq) {
            input.parse().map(PyClassClaraXOption::Eq)
        } else if lookahead.peek(kw::eq_int) {
            input.parse().map(PyClassClaraXOption::EqInt)
        } else if lookahead.peek(kw::extends) {
            input.parse().map(PyClassClaraXOption::Extends)
        } else if lookahead.peek(attributes::kw::freelist) {
            input.parse().map(PyClassClaraXOption::Freelist)
        } else if lookahead.peek(attributes::kw::frozen) {
            input.parse().map(PyClassClaraXOption::Frozen)
        } else if lookahead.peek(attributes::kw::get_all) {
            input.parse().map(PyClassClaraXOption::GetAll)
        } else if lookahead.peek(attributes::kw::hash) {
            input.parse().map(PyClassClaraXOption::Hash)
        } else if lookahead.peek(attributes::kw::immutable_type) {
            input.parse().map(PyClassClaraXOption::ImmutableType)
        } else if lookahead.peek(attributes::kw::mapping) {
            input.parse().map(PyClassClaraXOption::Mapping)
        } else if lookahead.peek(attributes::kw::module) {
            input.parse().map(PyClassClaraXOption::Module)
        } else if lookahead.peek(kw::name) {
            input.parse().map(PyClassClaraXOption::Name)
        } else if lookahead.peek(attributes::kw::ord) {
            input.parse().map(PyClassClaraXOption::Ord)
        } else if lookahead.peek(kw::rename_all) {
            input.parse().map(PyClassClaraXOption::RenameAll)
        } else if lookahead.peek(attributes::kw::sequence) {
            input.parse().map(PyClassClaraXOption::Sequence)
        } else if lookahead.peek(attributes::kw::set_all) {
            input.parse().map(PyClassClaraXOption::SetAll)
        } else if lookahead.peek(attributes::kw::new) {
            input.parse().map(PyClassClaraXOption::New)
        } else if lookahead.peek(attributes::kw::str) {
            input.parse().map(PyClassClaraXOption::Str)
        } else if lookahead.peek(attributes::kw::subclass) {
            input.parse().map(PyClassClaraXOption::Subclass)
        } else if lookahead.peek(attributes::kw::unsendable) {
            input.parse().map(PyClassClaraXOption::Unsendable)
        } else if lookahead.peek(attributes::kw::weakref) {
            input.parse().map(PyClassClaraXOption::Weakref)
        } else if lookahead.peek(attributes::kw::generic) {
            input.parse().map(PyClassClaraXOption::Generic)
        } else if lookahead.peek(attributes::kw::from_py_object) {
            input.parse().map(PyClassClaraXOption::FromPyObject)
        } else if lookahead.peek(attributes::kw::skip_from_py_object) {
            input.parse().map(PyClassClaraXOption::SkipFromPyObject)
        } else {
            Err(lookahead.error())
        }
    }
...
impl Parse for PyClassClaraXOptions {
    fn parse(input: ParseStream<'_>) -> syn::Result<Self> {
        let mut options: PyClassClaraXOptions = Default::default();

        for option in Punctuated::<PyClassClaraXOption, syn::Token![,]>::parse_terminated(input)? {
...
impl PyClassClaraXOptions {
    ...
    fn set_option(&mut self, option: PyClassClaraXOption) -> syn::Result<()> {
        ...
        match option {
            PyClassClaraXOption::Crate(krate) => set_option!(krate),
            PyClassClaraXOption::Dict(dict) => set_option!(dict),
            PyClassClaraXOption::Eq(eq) => set_option!(eq),
            PyClassClaraXOption::EqInt(eq_int) => set_option!(eq_int),
            PyClassClaraXOption::Extends(extends) => set_option!(extends),
            PyClassClaraXOption::Freelist(freelist) => set_option!(freelist),
            PyClassClaraXOption::Frozen(frozen) => set_option!(frozen),
            PyClassClaraXOption::GetAll(get_all) => set_option!(get_all),
            PyClassClaraXOption::ImmutableType(immutable_type) => {
                set_option!(immutable_type)
            }
            PyClassClaraXOption::Hash(hash) => set_option!(hash),
            PyClassClaraXOption::Mapping(mapping) => set_option!(mapping),
            PyClassClaraXOption::Module(module) => set_option!(module),
            PyClassClaraXOption::Name(name) => set_option!(name),
            PyClassClaraXOption::Ord(ord) => set_option!(ord),
            PyClassClaraXOption::RenameAll(rename_all) => set_option!(rename_all),
            PyClassClaraXOption::Sequence(sequence) => set_option!(sequence),
            PyClassClaraXOption::SetAll(set_all) => set_option!(set_all),
            PyClassClaraXOption::New(new) => set_option!(new),
            PyClassClaraXOption::Str(str) => set_option!(str),
            PyClassClaraXOption::Subclass(subclass) => set_option!(subclass),
            PyClassClaraXOption::Unsendable(unsendable) => set_option!(unsendable),
            PyClassClaraXOption::Weakref(weakref) => set_option!(weakref),
            PyClassClaraXOption::Generic(generic) => set_option!(generic),
            PyClassClaraXOption::SkipFromPyObject(skip_from_py_object) => {
                ensure_spanned!(
            ...
            PyClassClaraXOption::FromPyObject(from_py_object) => {
                ensure_spanned!(
...
pub fn build_py_class(
    ...
    if let Some(lt) = class.generics.lifetimes().next() {
        bail_spanned!(
            lt.span() => concat!(
                ...
                https://github.com/abdulwahed-sweden/clarax/v", env!("CARGO_PKG_VERSION"), "/class.html#no-lifetime-parameters"
            )
        );
    }
    ...
    ensure_spanned!(
        ...
        class.generics.span() => concat!(
            ...
            https://github.com/abdulwahed-sweden/clarax/v", env!("CARGO_PKG_VERSION"), "/class.html#no-generic-parameters"
        )
    );

    let mut field_options: Vec<(&syn::Field, FieldClaraXOptions)> = match &mut class.fields {
        syn::Fields::Named(fields) => fields
            .named
            .iter_mut()
            .map(
                |field| match FieldClaraXOptions::take_pyo3_options(&mut field.attrs) {
    ...
    if let Some(attr) = args.options.get_all {
        for (_, FieldClaraXOptions { get, .. }) in &mut field_options {
    ...
    if let Some(attr) = args.options.set_all {
        for (_, FieldClaraXOptions { set, .. }) in &mut field_options {
...
fn impl_class(
    ...
    field_options: Vec<(&syn::Field, FieldClaraXOptions)>,
    methods_type: PyClassMethodsType,
    ctx: &Ctx,
) -> Result<TokenStream> {
...
impl<'a> PyClassSimpleEnum<'a> {
    fn new(enum_: &'a mut syn::ItemEnum) -> syn::Result<Self> {
        ...
        fn extract_unit_variant_data(
            ...
            let options = EnumVariantClaraXOptions::take_pyo3_options(&mut variant.attrs)?;
...
trait EnumVariant {
    ...
    fn get_options(&self) -> &EnumVariantClaraXOptions;
...
struct PyClassEnumUnitVariant<'a> {
    ident: &'a syn::Ident,
    options: EnumVariantClaraXOptions,
...
#[derive(Clone, Default)]
struct EnumVariantClaraXOptions {
    name: Option<NameAttribute>,
    constructor: Option<ConstructorAttribute>,
}

enum EnumVariantClaraXOption {
    Name(NameAttribute),
    Constructor(ConstructorAttribute),
}

impl Parse for EnumVariantClaraXOption {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        let lookahead = input.lookahead1();
        if lookahead.peek(attributes::kw::name) {
            input.parse().map(EnumVariantClaraXOption::Name)
        } else if lookahead.peek(attributes::kw::constructor) {
            input.parse().map(EnumVariantClaraXOption::Constructor)
        } else {
            Err(lookahead.error())
        }
    }
...
impl EnumVariantClaraXOptions {
    fn take_pyo3_options(attrs: &mut Vec<syn::Attribute>) -> Result<Self> {
        let mut options = EnumVariantClaraXOptions::default();
    ...
    fn set_option(&mut self, option: EnumVariantClaraXOption) -> syn::Result<()> {
        macro_rules! set_option {
            ($key:ident) => {
                {
                    ensure_spanned!(
        ...
        match option {
            EnumVariantClaraXOption::Constructor(constructor) => set_option!(constructor),
            EnumVariantClaraXOption::Name(name) => set_option!(name),
        }
        Ok(())
    }
}
...
fn impl_complex_enum(
    ...
    for variant in variants {
        ...
        let variant_args = PyClassArgs {
            ...
            options: {
                let mut rigged_options: PyClassClaraXOptions = parse_quote!(extends = #cls, frozen);
