...
//!
//! There is a JSON per ClaraX proc macro (pyclass, pymodule, pyfunction...).
//!
...
use crate::utils::{ClaraXCratePath, PythonDoc, StrOrExpr};
...
pub fn module_introspection_code<'a>(
    pyo3_crate_path: &ClaraXCratePath,
    name: &str,
...
impl IntrospectionNode<'_> {
    fn emit(self, pyo3_crate_path: &ClaraXCratePath) -> TokenStream {
...
fn serialize_type_hint(hint: TokenStream, pyo3_crate_path: &ClaraXCratePath) -> TokenStream {
    quote! {{
...
impl ConcatenationBuilder {
    ...
    pub fn into_token_stream(self, pyo3_crate_path: &ClaraXCratePath) -> TokenStream {
    ...
    fn into_static(self, pyo3_crate_path: &ClaraXCratePath, ident: Ident) -> TokenStream {
        let mut elements = self.elements;
        if !self.current_string.is_empty() {
            elements.push(ConcatenationBuilderElement::String(self.current_string));
        }
