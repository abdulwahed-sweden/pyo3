...
ClaraX's attributes (`#[pyclass]`, `#[pymodule]`, etc.) are [procedural macros](https://doc.rust-lang.org/reference/procedural-macros.html), which means that they rewrite the source of the annotated item.
...
Since ClaraX uses `cdylib` for Python shared objects, it does not receive the pretty-print debug hooks in `rust-gdb` ([rust-lang/rust#96365](https://github.com/rust-lang/rust/issues/96365)).
...
This can be achieved in ClaraX by using a debugger like `rust-gdb` or `rust-lldb` with your Python interpreter.
...
2. Create a `.vscode/launch.json` file with a configuration that uses the LLDB Debug Launcher:
   ...
   ```json
   {
     ...
     "configurations": [
       ...
       {
         "name": "Launch Python with ClaraX",
         "type": "lldb",
         "request": "launch",
         "program": "${workspaceFolder}/.venv/bin/python",
         "args": ["${file}"],
...
4. Start debugging:
   - For attaching to a running Python process: First start the process, then select the "Debug ClaraX" configuration and click Start Debugging (F5).
     You'll be prompted to select the Python process to attach to.
   - For launching a Python script: Open your Python script, select the "Launch Python with ClaraX" configuration and click Start Debugging (F5).
   - For running with arguments: Select "Debug ClaraX with Args" (remember to edit the configuration with your actual script path and arguments).
   - For running tests: Select "Debug ClaraX Tests" (edit the test path as needed).

5. When debugging ClaraX code:
...
```json
{
  "name": "Debug ClaraX with Environment",
  "type": "lldb",
  "request": "launch",
  "program": "${workspaceFolder}/.venv/bin/python",
  "args": ["${file}"],
        ...
        if os.path.exists(launch_json_path):
            try:
                ...
                for config in existing_config.get("configurations", []):
                    if config.get("name") == "Debug ClaraX (Jupyter)":
                        config["pid"] = pid
...
3. In VS Code, select the "Debug ClaraX (Jupyter)" configuration and start debugging

## Thread Safety and Compiler Sanitizers

ClaraX attempts to match the Rust language-level guarantees for thread safety, but that does not preclude other code outside of the control of ClaraX or buggy code managed by a ClaraX extension module from creating a thread safety issue.
...
To use `ThreadSanitizer` with a library that depends on ClaraX, you will need to install a nightly Rust toolchain, along with the `rust-src` component, since you will need to compile the Rust standard library:
