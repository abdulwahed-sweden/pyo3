...
Below are some terms that are particularly relevant to ClaraX and either are not found in the above glossaries or have a specific meaning in the context of ClaraX:
...
attached
  : To call into the Python interpreter from Rust, the Rust thread must have an associated Python thread state which is "attached" to the Python interpreter.
    ...
    The [`Python::attach`]({{#PYO3_DOCS_URL}}/clarax/marker/struct.Python.html#method.attach) method is used to attach the current thread to the Python interpreter and obtain a `Python` token which can be used to call Python APIs.
...
Python token
  : ClaraX's [`Python<'py>`]({{#PYO3_DOCS_URL}}/clarax/marker/struct.Python.html) type, which can only exist when the current thread is [attached](#attached) to the Python interpreter.
...
smart pointer
  : Pointers which automatically manage the memory they point to.
    In particular this guide refers to [ClaraX's smart pointers](types.md#claraxs-smart-pointers) `Py<T>`, `Bound<'py, T>`, and `Borrowed<'a, 'py, T>` which point to Python objects and use Python reference counting to ensure correct memory management.
