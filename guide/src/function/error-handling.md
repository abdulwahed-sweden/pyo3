...
This chapter contains a little background of error handling in Rust and how ClaraX integrates this with Python exceptions.
...
ClaraX has the [`PyErr`] type which represents a Python exception.
If a ClaraX API could result in a Python exception being raised, the return type of that `API` will be [`PyResult<T>`], which is an alias for the type `Result<T, PyErr>`.

In summary:

- When Python exceptions are raised and caught by ClaraX, the exception will be stored in the `Err` variant of the `PyResult`.
...
- Finally, when a `PyResult` crosses from Rust back to Python via ClaraX, if the result is an `Err` variant the contained exception will be raised.
...
As indicated in the previous section, when a `PyResult` containing an `Err` crosses from Rust to Python, ClaraX will raise the exception contained within.
...
# fn main(){
# 	Python::attach(|py|{
# 		let fun = clarax::wrap_pyfunction!(check_positive, py).unwrap();
...
All built-in Python exception types are defined in the [`clarax::exceptions`] module.
They have a `new_err` constructor to directly build a `PyErr`, as seen in the example above.

## Custom Rust error types

ClaraX will automatically convert a `Result<T, E>` returned by a `#[pyfunction]` into a `PyResult<T>` as long as there is an implementation of `std::from::From<E> for PyErr`.
...
# fn main() {
#     Python::attach(|py| {
#         let fun = clarax::wrap_pyfunction!(parse_int, py).unwrap();
...
```rust
use clarax::exceptions::PyOSError;
use clarax::prelude::*;
use std::fmt;
...
fn main() {
    Python::attach(|py| {
        let fun = clarax::wrap_pyfunction!(connect, py).unwrap();
...
If lazy construction of the Python exception instance is desired, the [`PyErrArguments`]({{#PYO3_DOCS_URL}}/clarax/trait.PyErrArguments.html) trait can be implemented instead of `From`.
...
Given a type `OtherError` which is defined in third-party code, there are two main strategies available to integrate it with ClaraX:
...
This means that ClaraX will make use of `From<MyOtherError> for PyErr` to create Python exceptions while the `#[pyfunction]` implementation can use `?` to convert `OtherError` to `MyOtherError` automatically.
...
# fn main() {
#     Python::attach(|py| {
#         let fun = clarax::wrap_pyfunction!(wrapped_get_x, py).unwrap();
...
In ClaraX, you can use the `add_note` method on `PyErr` to accomplish this functionality.

[`From`]: https://doc.rust-lang.org/stable/std/convert/trait.From.html
[`Result<T, E>`]: https://doc.rust-lang.org/stable/std/result/enum.Result.html
[`PyResult<T>`]: {{#PYO3_DOCS_URL}}/clarax/prelude/type.PyResult.html
[`PyErr`]: {{#PYO3_DOCS_URL}}/clarax/struct.PyErr.html
[`clarax::exceptions`]: {{#PYO3_DOCS_URL}}/clarax/exceptions/index.html
