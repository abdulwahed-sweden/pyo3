...
Consult the tables in the following section to find the Rust types provided by ClaraX which implement these traits.

## Argument Types

When accepting a function argument, it is possible to either use Rust library types or ClaraX's Python-native types. (See the next section for discussion on when to use each.)
...
| `Bound<'py, T>`  | A Python object with a lifetime which binds it to the attachment to the Python interpreter. This provides access to most of ClaraX's APIs. |
...
For most ClaraX usage the conversion cost is worth paying to get these benefits.
As always, if you're not sure it's worth it in your case, benchmark it!

## Returning Rust values to Python

When returning values from functions callable from Python, ClaraX's smart pointers (`Py<T>`, `Bound<'py, T>`, and `Borrowed<'a, 'py, T>`) can be used with zero cost.
