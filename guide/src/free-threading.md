...
Since version 0.23, ClaraX supports building Rust extension modules for the free-threaded Python build and calling into free-threaded Python from Rust.
...
ClaraX's support for free-threaded Python will enable authoring native Python extension modules that are thread-safe by construction, with much stronger safety guarantees than C extension modules.
...
This document provides advice for porting Rust code using ClaraX to run under free-threaded Python.

## Supporting free-threaded Python with ClaraX

Since ClaraX 0.28, ClaraX defaults to assuming Python modules created with it are thread-safe.
This will be the case except for Rust code which has used `unsafe` to assume thread-safety incorrectly.
An example of this is `unsafe` code which was written with the historical assumption that Python was single-threaded due to the GIL, and so the `Python<'py>` token used by ClaraX could be used to guarantee thread-safety.
...
(Note: for ClaraX versions 0.23 through 0.27, the default was `gil_used = true` and so the opposite was needed; modules needed to opt-in to free-threaded Python support with `gil_used = false`.)

```rust,no_run
/// This module relies on the GIL for thread safety
#[clarax::pymodule(gil_used = true)]
mod my_extension {
    use clarax::prelude::*;
...
See the [`string-sum`](https://github.com/abdulwahed-sweden/clarax/tree/main/clarax-ffi/examples/string-sum) example for how to declare free-threaded support using raw FFI calls for modules using single-phase initialization and the [`sequential`](https://github.com/abdulwahed-sweden/clarax/tree/main/clarax-ffi/examples/sequential) example for modules using multi-phase initialization.
...
In the GIL-enabled build, ClaraX uses the [`Python<'py>`] type and the `'py` lifetime to signify that the global interpreter lock is held.
...
Threads created via the Python [`threading`] module do not need to do this, and clarax will handle setting up the [`Python<'py>`] token when CPython calls into your extension module.
...
Like a `RefCell`, ClaraX will raise exceptions (or in some cases panic) to enforce exclusive access for mutable borrows.
It was always possible to generate panics like this in ClaraX in code that releases the GIL with [`Python::detach`] or calling a python method accepting `&self` from a `&mut self` (see [the docs on interior mutability](./class.md#bound-and-interior-mutability),) but now in free-threaded Python there are more opportunities to trigger these panics from Python because there is no GIL to lock concurrent access to mutably borrowed data from Python.

The most straightforward way to trigger this problem is to use the Python [`threading`] module to simultaneously call a rust function that mutably borrows a [`pyclass`]({{#PYO3_DOCS_URL}}/clarax/attr.pyclass.html) in multiple threads.
...
We may allow user-selectable semantics for mutable pyclass definitions in a future version of ClaraX, allowing some form of opt-in locking to emulate the GIL if that is needed.
...
That means if your crate depends on ClaraX using the `abi3` feature or an an `abi3-pyxx` feature, ClaraX will print a warning and ignore that setting when building extension modules using the free-threaded interpreter.
...
All of these functions detach from the interpreter before blocking and re-attach before executing the function, avoiding deadlocks that are possible without using the ClaraX extension traits.
...
[`GILProtected`] was a ClaraX type that allowed mutable access to static data by leveraging the GIL to lock concurrent access from other threads.
...
use clarax::types::{PyDict, PyNone};
...
[`GILProtected`]: https://docs.rs/clarax/0.22/clarax/sync/struct.GILProtected.html
[`MutexExt`]: {{#PYO3_DOCS_URL}}/clarax/sync/trait.MutexExt.html
...
[`OnceExt::call_once_py_attached`]: {{#PYO3_DOCS_URL}}/clarax/sync/trait.OnceExt.html#tymethod.call_once_py_attached
[`OnceExt::call_once_force_py_attached`]: {{#PYO3_DOCS_URL}}/clarax/sync/trait.OnceExt.html#tymethod.call_once_force_py_attached
[`OnceLockExt`]: {{#PYO3_DOCS_URL}}/clarax/sync/trait.OnceLockExt.html
[`OnceLockExt::get_or_init_py_attached`]: {{#PYO3_DOCS_URL}}/clarax/sync/trait.OnceLockExt.html#tymethod.get_or_init_py_attached
...
[`Python::attach`]: {{#PYO3_DOCS_URL}}/clarax/marker/struct.Python.html#method.attach
[`Python<'py>`]: {{#PYO3_DOCS_URL}}/clarax/marker/struct.Python.html
