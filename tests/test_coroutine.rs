...
fn handle_windows(test: &str) -> String {
    ...
    "#;
    clarax::impl_::unindent::unindent(set_event_loop_policy) + &clarax::impl_::unindent::unindent(test)
}
...
fn test_async_function_returns_unit_none() {
    ...
    Python::attach(|py| {
        ...
        clarax::py_run!(py, returns_unit, &handle_windows(test));
    });
}
...
fn cancelled_coroutine() {
    Python::attach(|py| {
        ...
        let err = py
            .run(
                &CString::new(clarax::impl_::unindent::unindent(&handle_windows(test))).unwrap(),
                Some(&globals),
                None,
            )
            .unwrap_err();
