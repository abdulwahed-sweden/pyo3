...
/// Minimum Python version ClaraX supports.
    ...
    match interpreter_config.implementation {
        PythonImplementation::CPython => {
            let versions = SUPPORTED_VERSIONS_CPYTHON;
                ...
                "the configured Python interpreter version ({}) is lower than ClaraX's minimum supported version ({})",
            ...
            if interpreter_config.is_free_threaded() {
                let min_free_threaded_version = PythonVersion {
                    ...
                    "ClaraX does not support the free-threaded build of CPython versions below {}, the selected Python version is {}",
        ...
        PythonImplementation::PyPy => {
            let versions = SUPPORTED_VERSIONS_PYPY;
                ...
                interpreter_config.version >= versions.min,
                "the configured PyPy interpreter version ({}) is lower than ClaraX's minimum supported version ({})",
                interpreter_config.version,
                versions.min,
            );
            // ClaraX does not support abi3, so we cannot offer forward compatibility
        ...
        PythonImplementation::GraalPy => {
            let versions = SUPPORTED_VERSIONS_GRAALPY;
                ...
                interpreter_config.version >= versions.min,
                "the configured GraalPy interpreter version ({}) is lower than ClaraX's minimum supported version ({})",
                interpreter_config.version,
                versions.min,
            );
