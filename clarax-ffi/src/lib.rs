...
//! It is meant for advanced users only - regular ClaraX users shouldn't
...
//!
//! ClaraX uses `rustc`'s `--cfg` flags to enable or disable code used for different Python versions.
//! If you want to do this for your own crate, you can do so with the [`clarax-build-config`] crate.
//!
...
//! - ClaraX: PyPy and GraalPy are not supported. CPython only.
//!
...
//! ```
//!
//! To use these attributes, add [`clarax-build-config`] as a build dependency in
//! your `Cargo.toml`:
//!
//! ```toml
//! [build-dependencies]
#![doc = concat!("clarax-build-config =\"", env!("CARGO_PKG_VERSION"),  "\"")]
//! ```
//!
...
//!
//! `clarax-ffi` supports the following Python distributions:
...
//!
//! [dependencies]
#![doc = concat!("clarax-ffi = \"", env!("CARGO_PKG_VERSION"),  "\"")]
//!
//! [build-dependencies]
...
#![doc = concat!("clarax_build_config = \"", env!("CARGO_PKG_VERSION"),  "\"")]
//! ```
//!
//! If you need to use conditional compilation based on Python version or how
//! Python was compiled, you need to add `clarax-build-config` as a
...
//! `clarax_build_config::use_pyo3_cfgs()` gets called at build time:
//!
//! **`build.rs`**
//! ```rust,ignore
//! fn main() {
...
//! /// If we used ClaraX's proc macros they'd handle all of this boilerplate for us :)
...
//! `sequential` project located in the `examples` directory at the root of the `clarax-ffi` crate
//! for a worked example of how to this using `clarax-ffi`.
//!
...
//! you can take a look at the [`orjson`] library as an example on how to use `clarax-ffi` directly.
...
//! [`clarax-build-config`]: https://docs.rs/clarax-build-config
...
///
/// It can be used on all Rust versions supported by ClaraX, unlike c"" literals which
/// were stabilised in Rust 1.77.
///
/// Due to the nature of ClaraX making heavy use of C FFI interop with Python, it is
/// common for ClaraX to use CStr.
///
/// Examples:
///
/// ```rust,no_run
