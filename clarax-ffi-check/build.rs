fn main() {
    ...
    let doc_dir = std::path::Path::new(&out_dir)
        .join(&target)
        .join("doc")
        .join("clarax_ffi_check_definitions");
    ...
    let status = std::process::Command::new("cargo")
        .args(["doc", "-p", "clarax-ffi-check-definitions", "--no-deps"])
    ...
    println!("cargo:rerun-if-changed=../clarax-ffi");

    // Because `clarax-ffi` is a dependency, libpython is linked, this ensures `main.rs` can run.
